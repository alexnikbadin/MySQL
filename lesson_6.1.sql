...
SELECT COUNT(from_user_id), from_user_id FROM messages
WHERE to_user_id IN (
                      SELECT to_user_id FROM friend_requests 
                      WHERE to_user_id = 1 AND request_type = 1
                      ) 
AND is_delivered = 1 GROUP BY from_user_id; -- наибольшее количество доставленных сообщений поступило к to_user_id 1
                                            -- было отправлено от from_user_id 2
...
SELECT COUNT(user_id), user_id FROM posts_likes WHERE like_type = 0 GROUP BY user_id; -- user_id 1


SELECT COUNT(from_user_id), from_user_id FROM messages GROUP BY from_user_id; -- from_user_id 7

SELECT COUNT(from_user_id), from_user_id FROM friend_requests 
WHERE to_user_id = 2 OR to_user_id = 0 GROUP BY from_user_id; -- from_user_id 1 
